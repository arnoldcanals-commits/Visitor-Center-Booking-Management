...
export default function AddBillItemModal({ bill, item = null, onClose }) {
  const { reload } = useContext(AdminDataContext);
    ...
    const loadData = async () => {
      try {
        const [guestRes, feeRes] = await Promise.all([
          api.get("api/site_admin/guests/"),
          api.get("api/site_admin/fee_types/"),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = item
        ? `api/site_admin/bill_items/${item.id}/update/`
        : "api/site_admin/bill_items/";
